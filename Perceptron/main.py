...
for blank in range(iterations):
    predictions, weights, c = pe.voted_evaluate(training_data, training_data, learning_rate, T)
    acc, counts = pe.accuracy(training_data, predictions)
...
acc_basic = []
for blank in range(iterations):
    predictions, weights = pe.evaluate(training_data, training_data, learning_rate, T)
    acc, counts = pe.accuracy(training_data, predictions)
...
acc_averaged = []
for blank in range(iterations):
    predictions, weights = pe.averaged_evaluate(training_data, training_data, learning_rate, T)
    acc, counts = pe.accuracy(training_data, predictions)
